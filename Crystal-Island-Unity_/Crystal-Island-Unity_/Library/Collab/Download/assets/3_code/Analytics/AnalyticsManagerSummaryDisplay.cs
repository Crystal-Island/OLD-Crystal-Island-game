    ...
    public class AnalyticsManagerSummaryDisplay : VCBehaviour<AnalyticsManager>
    {
        ...
        private void gameStateChanged(int oldState, int newState)
        {
            if (newState == (int)PolymoneyGameFlow.FlowStates.SUMMARY)
            {
                ...
                monthPool.releaseAll();
                foreach (PolymoneyMonthData month in model.Months)
                {
                    Transform monthDisplay = monthPool.pop();
                    VC<PolymoneyMonthData>.addModelToAllControllers(month, monthDisplay.gameObject, true);
                    monthDisplay.gameObject.SetActive(true);
                }
            }
        ...
        {
            Application.OpenURL(Localisation.instance.getLocalisedText("moreInfoURL"));
        }
