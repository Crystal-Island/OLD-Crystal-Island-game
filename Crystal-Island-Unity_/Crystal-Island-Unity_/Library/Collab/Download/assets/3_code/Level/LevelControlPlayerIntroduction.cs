    ...
    public class LevelControlPlayerIntroduction : VCBehaviour<Level>
    {
        ...
        private IEnumerator introductionRoutine()
        {
            if (model.authoritativePlayer.Mayor)
            {
                ...
                while (Alert.open)
                    yield return null;
                Alert.info("tutoMSurvive2", new Alert.AlertParams { useLocalization = true, title = "tutoMSurviveTitle", closeText = "btnOk" });
                while (Alert.open)
                    yield return null;
            ...
            else
            {
                ...
                Alert.info("tutoPIntro1", new Alert.AlertParams { useLocalization = true, title = "tutoMIntroduceTitle", closeText = "btnOk" });
                while (Alert.open)
                    yield return null;
                Alert.info("tutoPIntro2", new Alert.AlertParams { useLocalization = true, title = "tutoMIntroduceTitle", closeText = "btnOk" });
                while (Alert.open)
                    yield return null;
                Alert.info("tutoPIntro3", new Alert.AlertParams { useLocalization = true, title = "tutoMIntroduceTitle", closeText = "btnOk" });
                while (Alert.open)
                    yield return null;
